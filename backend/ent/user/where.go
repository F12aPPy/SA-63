...
// HasCARE applies the HasEdge predicate on the "CARE" edge.
func HasCARE() predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(CARETable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, CARETable, CAREColumn),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}

// HasCAREWith applies the HasEdge predicate on the "CARE" edge with a given conditions (other predicates).
func HasCAREWith(preds ...predicate.Antenatal) predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(CAREInverseTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, CARETable, CAREColumn),
		)
