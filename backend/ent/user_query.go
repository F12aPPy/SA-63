...
// QueryCARE chains the current query on the CARE edge.
func (uq *UserQuery) QueryCARE() *AntenatalQuery {
	query := &AntenatalQuery{config: uq.config}
...
//  WithCARE tells the query-builder to eager-loads the nodes that are connected to
// the "CARE" edge. The optional arguments used to configure the query builder of the edge.
func (uq *UserQuery) WithCARE(opts ...func(*AntenatalQuery)) *UserQuery {
	query := &AntenatalQuery{config: uq.config}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withCARE; query != nil {
		...
		query.Where(predicate.Antenatal(func(s *sql.Selector) {
			s.Where(sql.InValues(user.CAREColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_care
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_care" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_care" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.CARE = append(node.Edges.CARE, n)
		}
	}
